...
public class VehicleDetails extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		else if ("vehicletype".equals(paths[1])&& paths.length<=3) {
		...
		} else if ("manufacturer".equals(paths[1])&& paths.length<=3) {
			Integer type = Integer.valueOf(paths[2]);
		...
		} else if ("model_name".equals(paths[1])&&  paths.length<=3) {
           Integer type=Integer.valueOf(paths[2]);
		 ...
		 */
		else if ("welcome".equals(paths[1]) &&  paths.length<=3) {
			...
			pw.write(json);
		} else if ("vehicletypedata".equals(paths[1]) &&  paths.length<=3) {
			PrintWriter pw = response.getWriter();
			Integer type=Integer.valueOf(paths[2]);
			...
			pw.write(json);
		} else if ("manufacturerdata".equals(paths[1]) &&  paths.length<=3) {
			PrintWriter pw = response.getWriter();
			Integer type=Integer.valueOf(paths[2]);
			...
			pw.write(json);
		} else if ("getvehicle".equals(paths[1]) &&  paths.length<=4) {
			// if the path contain getvehicle as a value we will return the details of vehicle as
			// String as a json format
			Integer manu=Integer.valueOf(paths[2]);
			Integer model=Integer.valueOf(paths[3]);
